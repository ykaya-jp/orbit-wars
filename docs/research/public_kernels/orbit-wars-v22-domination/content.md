...
```python
"""
Orbit Wars Agent - v22 "Domination"

Major improvements over v21:
...
1. N-WAY COORDINATED ATTACKS: Instead of just 2-planet coordination, collect
   ...
   This allows taking down heavily fortified enemy planets.
...
def _infer_fleet_target(fleet_x, fleet_y, fleet_angle, planets, angular_velocity, fleet_ships):
    ...
    Checks both current planet position AND predicted intercept position.
...
def agent(obs):
    ...
    for fleet in raw_fleets:
        ...
        if fleet_owner == player:
            ...
            # Classify threat to our planets
            best_p = None
            best_cos = 0.4
            for p in my_planets:
                dx, dy = p['x'] - fleet_x, p['y'] - fleet_y
                ...
                if cos_a > best_cos:
                    ...
                    best_p = p
            if best_p:
                threat_map[best_p['id']] = threat_map.get(best_p['id'], 0) + fleet_ships
