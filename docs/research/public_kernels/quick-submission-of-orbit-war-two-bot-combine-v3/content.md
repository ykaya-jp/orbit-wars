...
def agent_Japan(obs):
    """
    Orbit Wars agent: decides which ships to send where.
    Implements multiple candidate strategies (aggressive, conservative, etc.),
    simulates them, and picks the one with the highest evaluation score.

    The final set of moves is truncated to a random 3 or 4 ships – this is
    intentional to avoid being predictable and to match top‑tier bot behaviour
    observed in Orbit Wars (many strong bots send only 3‑4 fleets per turn).
    """
    ...
    # Game constants
    # -------------------------------------------------------------------
    MAX_SHIP_SPEED = 6.0                # maximum fleet speed (log scaling)
    EPISODE_STEPS = 500                 # total steps per game
    FIRE_THRESHOLD = 10                 # minimum ships left on a planet before it can fire
    MIN_FIRE_SHIPS = 3                  # minimum ships in a launched fleet
    DEFENSE_MIN_DEFICIT = 10            # ignore defensive deficit if smaller
    DEFENSE_MIN_PROD = 2                # defend only planets with production >= 2
    SIM_HORIZON = 20                    # how many steps we simulate ahead
    TIME_BUDGET = 0.7                   # seconds allowed for the whole agent (cut short if needed)

    # -------------------------------------------------------------------
    # Fleet speed formula: scales with fleet size using a log curve.
    # Single ship always moves at speed 1.0; 1000 ships approach MAX_SHIP_SPEED.
    # -------------------------------------------------------------------
    def _fleet_speed(n):
        if n <= 1:
            return 1.0
        s = 1.0 + (MAX_SHIP_SPEED - 1.0) * (math.log(n) / math.log(1000)) ** 1.5
        return min(s, MAX_SHIP_SPEED)

    # -------------------------------------------------------------------
    # Check if a line segment (x1,y1)->(x2,y2) intersects the sun.
    # Used to prevent ships from flying through (and being destroyed by) the sun.
    # -------------------------------------------------------------------
    def _line_hits_sun(x1, y1, x2, y2):
        dx, dy = x2 - x1, y2 - y1
        l2 = dx * dx + dy * dy
        if l2 == 0.0:
            return math.hypot(x1 - CENTER, y1 - CENTER) < SUN_RADIUS
        # Project the sun's centre onto the line, clamp to segment.
        t = max(0.0, min(1.0, ((CENTER - x1) * dx + (CENTER - y1) * dy) / l2))
        px, py = x1 + t * dx, y1 + t * dy
        return math.hypot(px - CENTER, py - CENTER) < SUN_RADIUS

    # -------------------------------------------------------------------
    # Distance from point (px,py) to line segment (x1,y1)->(x2,y2).
    # Used to detect fleet collisions with planets.
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # Predict the position of a planet at a future time, considering its
    # initial position (from initial_planets_obs) and the angular velocity.
    # If the planet is too far out, its rotation is negligible (static).
    # -------------------------------------------------------------------
    def _predict_planet_pos(ipl, ang_vel, step, t_future):
        ix, iy, r_planet = ipl[2], ipl[3], ipl[4]
        dx = ix - CENTER
        dy = iy - CENTER
        r_orb = math.sqrt(dx * dx + dy * dy)
        # Planets beyond ROTATION_RADIUS_LIMIT are considered stationary.
        if r_orb + r_planet >= ROTATION_RADIUS_LIMIT:
            return ix, iy
        init_angle = math.atan2(dy, dx)
        theta = init_angle + ang_vel * (step + t_future)
        return CENTER + r_orb * math.cos(theta), CENTER + r_orb * math.sin(theta)

    # -------------------------------------------------------------------
    # How many steps will it take for a fleet of 'ships_est' to travel 'dist'?
    # -------------------------------------------------------------------
    def _travel_time(dist, ships_est):
        v = _fleet_speed(ships_est)
        return max(1, int(math.ceil(dist / max(v, 0.1))))

    # -------------------------------------------------------------------
    # Helper: priority multiplier based on game phase and whether we are losing.
    # Earlier game or losing -> more aggressive expansions.
    # -------------------------------------------------------------------
    def _priority_by_phase(step, tp_owner, tp_production, is_losing):
        if step < 100:
            base = 3.0 if tp_owner == -1 else (1.5 if tp_production >= 4 else 1.0)
        elif step < 300:
            base = 2.0 if tp_owner == -1 else (2.0 if tp_production >= 4 else 1.5)
        else:
            base = 1.0 if tp_owner == -1 else (3.0 if tp_production >= 4 else 2.0)
        if is_losing and tp_owner not in (-1, None):
            base *= 1.5
        return base

    # -------------------------------------------------------------------
    # Multi‑factor target value (V22 + V28 enhancements).
    # Incorporates phase awareness, enemy focus, neutral de‑prioritisation
    # in late game, and special handling for 4‑player games (weakest player bonus).
    # -------------------------------------------------------------------
    def _multi_factor_value(target, t_est, remaining_turns, is_static, is_early, is_late,
                            is_finishing, is_behind, is_dominating, owner_player, num_players,
                            step=0, weakest_player=None):
        # Base value: production × remaining profitable turns after travel.
        turns_profit = max(1, remaining_turns - t_est)
        value = target.production * turns_profit

        # Static planets (far from sun) are slightly more valuable.
        if is_static:
            value *= 1.4 if target.owner == -1 else 1.55
        else:
            value *= 0.9 if is_early else 1.0

        # Hostile planet bonus, stronger in mid/late game (V28).
        if target.owner not in (-1, owner_player):
            if is_early:
                value *= 1.45
            elif step < 200:      # mid game
                value *= 2.2      # increased from 1.85
            else:                 # late game
                value *= 3.0      # extreme focus on taking enemy planets

        # Neutral planet de‑valuation after turn 200 (V28).
        if target.owner == -1:
            if is_early:
                value *= 1.2
            elif step >= 200:
                value *= 0.3      # ignore neutral if late

        # Finishing blow: if we are dominant and enemy is weak, bonus.
        if is_finishing and target.owner not in (-1, owner_player):
            value *= 1.15

        # If we are behind, neutral planets are slightly less attractive.
        if is_behind and target.owner == -1 and not is_static:
            value *= 0.92

        # Weakest player bonus (4‑player kingmaking avoidance).
        if weakest_player is not None and target.owner == weakest_player:
            value *= 1.15

        # Late game (few turns left) additional enemy planet bonus.
        if is_late and target.owner not in (-1, owner_player):
            value *= 1.1

        return value

    # -------------------------------------------------------------------
    # Margin calculation: how many extra ships to send beyond the bare minimum.
    # Higher for static planets, 4‑player games, long travel times, and finishing moves.
    # -------------------------------------------------------------------
    def _compute_margin(target, t_est, is_static, is_four_player, is_finishing, player):
        if target.owner == -1:
            margin = min(8, 2 + target.production * 2)
        else:
            margin = min(12, 3 + target.production * 2)
        if is_static:
            margin += 4
        if is_four_player:
            margin += 3
        if t_est > 18:
            margin += min(8, t_est // 3)
        if is_finishing and target.owner not in (-1, player):
            margin += 3
        return margin

    # -------------------------------------------------------------------
    # Move generation logic (V11 parametric version).
    # Produces a list of [source_planet_id, angle, ships] for our fleets.
    # Parameters:
    #   enemy_margin_mult : how many times the base margin to send (affects aggression)
    #   room_factor       : multiplier for the "room" (how many total ships can be sent to a target)
    #   fire_threshold    : planets with fewer ships than this cannot launch attacks
    # -------------------------------------------------------------------
    def gen_moves(obs, player, enemy_margin_mult, room_factor, fire_threshold=FIRE_THRESHOLD):
        ...
        # Convert raw data to Planet objects.
        planets = [Planet(*p) for p in raw_planets]
        initial_by_id = {p[0]: p for p in raw_initial}
        my_planets = [p for p in planets if p.owner == player]
        if not my_planets:
            return []   # dead – no moves possible

        # Production balance to determine if we are losing.
        my_prod = sum(p.production for p in my_planets)
        enemy_prod = sum(p.production for p in planets if p.owner not in (player, -1))
        is_losing = enemy_prod > my_prod * 1.1

        # ---- Defensive analysis: detect incoming enemy fleets ----
        incoming = {p.id: 0 for p in my_planets}
        earliest_arrival = {p.id: 999 for p in my_planets}
        for f in raw_fleets:
            if f[1] == player:
                continue   # our own fleets are not threats
            fx, fy, fang, fships = f[2], f[3], f[4], f[6]
            cx_, cy_ = math.cos(fang), math.sin(fang)
            for mp in my_planets:
                rx, ry = mp.x - fx, mp.y - fy
                d_line = rx * cx_ + ry * cy_    # distance along the fleet's direction
                if d_line <= 0:
                    continue   # fleet is moving away from this planet
                side = abs(rx * (-cy_) + ry * cx_)   # perpendicular distance
                if side < mp.radius + 0.5:
                    incoming[mp.id] += fships
                    v = _fleet_speed(fships)
                    t_arr = max(1, int(math.ceil(d_line / max(v, 0.1))))
                    if t_arr < earliest_arrival[mp.id]:
                        earliest_arrival[mp.id] = t_arr
                    break

        # Remaining ships on our planets (after subtracting planned moves).
        remaining = {p.id: p.ships for p in my_planets}
        # Reserve ships: keep enough to defend against incoming attacks + production.
        reserves = {mp.id: max(5, incoming[mp.id] + mp.production) for mp in my_planets}
        moves = []

        # ---- Game state classification (V22 lightweight mode detection) ----
        total_my = sum(p.ships for p in my_planets)
        total_enemy = sum(p.ships for p in planets if p.owner not in (-1, player))
        alive_enemies = {p.owner for p in planets if p.owner not in (-1, player)}
        num_players = 1 + len(alive_enemies)
        is_four_player = num_players >= 3
        is_early = step < 80
        is_late = remaining_turns < 60
        is_finishing = (total_my > total_enemy * 1.3) and len(alive_enemies) >= 1 and step > 50
        is_behind = total_my * 1.1 < total_enemy
        is_dominating = total_my > total_enemy * 1.5

        # Identify weakest opponent (used for kingmaking avoidance in 4‑player).
        weakest_player = None
        if is_four_player and alive_enemies:
            enemy_strength = {}
            for p in planets:
                if p.owner in alive_enemies:
                    enemy_strength[p.owner] = enemy_strength.get(p.owner, 0) + p.ships + p.production * 50
            if enemy_strength:
                weakest_player = min(enemy_strength, key=enemy_strength.get)

        # ---- Defensive reinforcement: help planets about to be overwhelmed ----
        for mp in my_planets:
            t = earliest_arrival[mp.id]
            if t >= 999:
                continue
            if mp.production < DEFENSE_MIN_PROD:
                continue
            future_own = mp.ships + mp.production * t
            deficit = incoming[mp.id] - future_own + 1
            if deficit < DEFENSE_MIN_DEFICIT:
                continue
            # Try to send reinforcements from the nearest friendly planet.
            others = [p for p in my_planets if p.id != mp.id]
            others.sort(key=lambda p: math.hypot(p.x - mp.x, p.y - mp.y))
            for src in others:
                if _line_hits_sun(src.x, src.y, mp.x, mp.y):
                    continue   # would fly into the sun – abort
                d = math.hypot(src.x - mp.x, src.y - mp.y)
                t_supply = _travel_time(d, max(20, deficit + 5))
                if t_supply > t:   # cannot arrive in time
                    continue
                if remaining[src.id] - deficit < max(5, src.production * 2):
                    continue   # not enough ships
                angle = math.atan2(mp.y - src.y, mp.x - src.x)
                moves.append([src.id, angle, int(deficit)])
                remaining[src.id] -= deficit
                break

        # ---- Offensive candidate generation ----
        candidates = []
        for mp in my_planets:
            if remaining[mp.id] < fire_threshold:
                continue
            for tp in planets:
                if tp.id == mp.id or tp.owner == player:
                    continue
                if tp.id in comet_ids:
                    continue   # comets are not worth targeting (they explode)
                ipl = initial_by_id.get(tp.id)
                if ipl is None:
                    # Fallback: use current planet data (risky but necessary)
                    ipl = [tp.id, tp.owner, tp.x, tp.y, tp.radius, tp.ships, tp.production]

                # Estimate travel time and future position.
                dx0 = tp.x - mp.x
                dy0 = tp.y - mp.y
                dist0 = math.hypot(dx0, dy0)
                if dist0 <= 0:
                    continue
                t_est = _travel_time(dist0, 50)   # 50 is a rough ship estimate for speed
                tx, ty = _predict_planet_pos(ipl, ang_vel, step, t_est)
                dx, dy = tx - mp.x, ty - mp.y
                ...
                if tp.owner == -1:
                    future_garrison = tp.ships
                else:
                    future_garrison = tp.ships + tp.production * t_est
                needed = future_garrison + 1

                # Refine travel time with the actual needed fleet size.
                t2 = _travel_time(dist, max(needed, 20))
                if t2 != t_est:
                    t_est = t2
                    tx, ty = _predict_planet_pos(ipl, ang_vel, step, t_est)
                    dx, dy = tx - mp.x, ty - mp.y
                ...
                # Discard if travel takes too long.
                if t_est >= remaining_turns - 2:
                    continue
                if _line_hits_sun(mp.x, mp.y, tx, ty):
                    continue   # sun collision

                angle = math.atan2(dy, dx)

                # Determine if target planet is static (outer ring).
                is_static = (math.hypot(tp.x - CENTER, tp.y - CENTER) + tp.radius >= ROTATION_RADIUS_LIMIT)

                # Calculate multi‑factor value for the target.
                mf_value = _multi_factor_value(
                    tp, t_est, remaining_turns, is_static, is_early, is_late,
                    is_finishing, is_behind, is_dominating, player, num_players,
                    step=step, weakest_player=weakest_player,
                )
                # Bonus if we are losing and target is enemy‑owned.
                if is_losing and tp.owner not in (-1, player):
                    mf_value *= 1.5

                # Score = value divided by cost (ships + distance).
                score = mf_value / (needed + 0.2 * dist + 1.0)
                candidates.append((score, mp.id, tp.id, angle, needed, t_est))

        # ---- Select best offensive moves respecting ship limits ----
        candidates.sort(key=lambda x: -x[0])
        target_sent = {}   # track how many ships already allocated to each target
        for score, src_id, tgt_id, angle, needed, t_est in candidates:
            avail = remaining[src_id] - reserves[src_id]
            if avail < MIN_FIRE_SHIPS:
                continue
            already = target_sent.get(tgt_id, 0)
            # Room: how many total ships we can still send to this target.
            room = int(needed * room_factor) + 3 - already
            if room <= 0:
                continue
            tgt_planet = next(p for p in planets if p.id == tgt_id)
            is_static_t = (math.hypot(tgt_planet.x - CENTER, tgt_planet.y - CENTER)
                        + tgt_planet.radius >= ROTATION_RADIUS_LIMIT)
            # Compute margin (extra ships) for this specific target.
            margin = _compute_margin(tgt_planet, t_est, is_static_t, is_four_player,
                                    is_finishing, player)
            send = min(avail, room, needed + margin)
            if send < MIN_FIRE_SHIPS:
                continue
            # Avoid under‑sending against defended enemy planets if we still can't meet the 80% threshold.
            if (send < needed + 1 and tgt_planet.owner != -1
                    and already + send < int(needed * 0.8)):
                continue
            moves.append([src_id, angle, int(send)])
            remaining[src_id] -= send
            target_sent[tgt_id] = already + send

            # V55: Limit number of fleets per planet to match top‑tier bot behaviour.
            # 4‑player games -> max 3; early game -> 2; plenty of ships left -> 3; else 2.
            if is_four_player:
                max_moves = 3
            elif is_early and step < 40:
                max_moves = 2
            elif remaining[src_id] > 50:
                max_moves = 3
            else:
                max_moves = 2
    ...
    # ===================================================================
    # Lightweight simulator: runs simulation for SIM_HORIZON steps.
    # It assumes enemies do nothing (noop). The agent's moves are injected.
    # Planets rotate based on their **initial** positions to avoid drift.
    # ===================================================================
    def simulate(planets_init, fleets_init, my_moves, ang_vel, step_offset, comet_pids, n_steps, my_player, initial_planets_obs=None):
        planets = [list(p) for p in planets_init]
        fleets = [list(f) for f in fleets_init]
        next_fleet_id = max((f[0] for f in fleets), default=-1) + 1

        # Store initial positions for correct rotation calculation.
        initial_pos = {}
        initial_radii = {}
        src = initial_planets_obs if initial_planets_obs else planets_init
        for p in src:
            initial_pos[p[0]] = (p[2], p[3])
            initial_radii[p[0]] = p[4]

        # Convert our moves into fleets.
        for move in my_moves:
            if len(move) != 3:
                continue
            from_id, angle, ships = move
            ships = int(ships)
            from_p = next((p for p in planets if p[0] == from_id), None)
            if from_p and from_p[1] == my_player and from_p[5] >= ships and ships > 0:
                from_p[5] -= ships
                # Place the fleet just outside the planet's radius in the given direction.
                sx = from_p[2] + math.cos(angle) * (from_p[4] + 0.1)
                sy = from_p[3] + math.sin(angle) * (from_p[4] + 0.1)
                fleets.append([next_fleet_id, my_player, sx, sy, angle, from_id, ships])
                next_fleet_id += 1

        # Step‑by‑step simulation.
        for sim_step in range(n_steps):
            actual_step = step_offset + sim_step
            ...
            # 2. Fleet movement & sun collision; check arrival against planets.
            new_fleets = []
            combat = {p[0]: [] for p in planets}
            for f in fleets:
                speed = _fleet_speed(f[6])
                ox, oy = f[2], f[3]   # previous position
                ...
                if not (0 <= f[2] <= BOARD_SIZE and 0 <= f[3] <= BOARD_SIZE):
                    continue
                # Sun collision -> lost.
                if _point_to_seg_dist(CENTER, CENTER, ox, oy, f[2], f[3]) < SUN_RADIUS:
                    continue
                # Check arrival at any planet.
                hit = False
                for p in planets:
                    if _point_to_seg_dist(p[2], p[3], ox, oy, f[2], f[3]) < p[4]:
                        combat[p[0]].append(f)
                        hit = True
                        break
                if not hit:
            ...
            # 3. Planet rotation (skip comets).
            for p in planets:
                if p[0] in comet_pids:
                    continue
                init = initial_pos.get(p[0])
                if not init:
                    continue
                ix, iy = init
                dx = ix - CENTER
                ...
                if r + p[4] >= ROTATION_RADIUS_LIMIT:
                    continue
                init_a = math.atan2(dy, dx)
                cur_a = init_a + ang_vel * actual_step
                p[2] = CENTER + r * math.cos(cur_a)
                p[3] = CENTER + r * math.sin(cur_a)

            # 4. Combat resolution per planet.
            for pid, fs in combat.items():
                if not fs:
                    continue
                p = next((pp for pp in planets if pp[0] == pid), None)
                if p is None:
                    continue
                owner_ships = {}
                for f in fs:
                    owner_ships[f[1]] = owner_ships.get(f[1], 0) + f[6]
                sorted_p = sorted(owner_ships.items(), key=lambda x: -x[1])
                top, ts = sorted_p[0]
                if len(sorted_p) > 1:
    ...
    # -------------------------------------------------------------------
    # State evaluation: calculates a heuristic score from the perspective of
    # 'my_player'. Takes into account total ships, production, planet count,
    # and weights production by remaining turns.
    # -------------------------------------------------------------------
    def evaluate_state(planets, fleets, my_player, remaining_turns):
        my_s = sum(p[5] for p in planets if p[1] == my_player) + sum(f[6] for f in fleets if f[1] == my_player)
        enemy_s = sum(p[5] for p in planets if p[1] not in (-1, my_player)) + sum(f[6] for f in fleets if f[1] not in (-1, my_player))
        my_p = sum(p[6] for p in planets if p[1] == my_player)
        enemy_p = sum(p[6] for p in planets if p[1] not in (-1, my_player))
        my_pcount = sum(1 for p in planets if p[1] == my_player)
        enemy_pcount = sum(1 for p in planets if p[1] not in (-1, my_player))
        return (
            (my_s - enemy_s)
            + (my_p - enemy_p) * remaining_turns * 0.5
            + (my_pcount - enemy_pcount) * 50
        )
    ...
    # Generate several candidate strategies by varying margin and room factor.
    candidates = []
    # V11 default: moderate margin (×4), room ×1.2
    candidates.append(("v11_def", gen_moves(obs, player, 4, 1.2)))
    # V13: aggressive (margin ×6, room ×1.5)
    candidates.append(("v13", gen_moves(obs, player, 6, 1.5)))
    # Very aggressive
    candidates.append(("v_aggr", gen_moves(obs, player, 8, 1.8)))
    # Conservative
    candidates.append(("v_cons", gen_moves(obs, player, 2, 1.0)))
    # Do nothing baseline
    ...
    # Try each candidate, simulate, and keep the best.
    for name, moves in candidates:
        if time.time() - t_start > TIME_BUDGET:
            break   # out of time, use best so far
        try:
            planets_after, fleets_after = simulate(
                raw_planets, raw_fleets, moves, ang_vel, step, comet_pids, SIM_HORIZON, player,
                initial_planets_obs=raw_initial,
            )
        except Exception:
            continue   # skip if simulation fails (shouldn't happen)
        s = evaluate_state(planets_after, fleets_after, player, remaining_turns - SIM_HORIZON)
        if s > best_score:
            best_score = s
            best_moves = moves
            best_name = name

    # V55 / V54 final tweak: top bots randomly limit fleets to 3 or 4.
    # This avoids being perfectly predictable and matches observed top‑level behaviour.
    Jj = random.choice([3, 4])
    # Debug print (visible in Kaggle log if needed).
    print(best_moves[:Jj], '----Japan-----')
    return best_moves[:Jj]
...
def plan_moves(world, deadline=None):
    ...
    def finalize_moves():
        ...
        print(final_moves[:Jj],'----MAIN-----', final_moves)

        return final_moves[:Jj]
...
def agent(obs, config=None):
    ...
    sleep1 = [v for v in range( 7,18)]
    sleep2 = [v for v in range(27,38)]
    sleep3 = [v for v in range(47,54)]
    sleep4 = [v for v in range(70,77)]
    sleep7 = [v for v in range(87,97)]
    sleep8 = [v for v in range(107,117)]
    sleep9 = [v for v in range(127,137)]

    sleeps = sleep1 + sleep2 + sleep3 + sleep4 + sleep7

    if step in sleeps: return []

    if step > 30 and 1 == random.choice([1,2,3,4,7]): return []

    if 'A' == random.choice(['A','B']):
        if 1 == random.choice([1,2,2,2,2,2,]): return agent_Japan(obs)
    else:
        if 1 == random.choice([1,2,2,2,2,2,2,2,2]): return agent_Japan(obs)
