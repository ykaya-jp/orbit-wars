...
def preferred_send(target, base_needed, arrival_turns, src_available, world, modes, policy):
    ...
    margin = 0
    if target.owner == -1:    # simplified margin calculation, just a basic buffer
    ...
    for target_id, options in source_options_by_target.items():
        ...
        for i in range(len(top_options)):
            for j in range(i + 1, len(top_options)):
                ...
                missions.append(Mission(kind="swarm", score=pair_score, target_id=target_id, turns=joint_turn, options=[first, second]))    missions.sort(key=lambda item: -item.score)
