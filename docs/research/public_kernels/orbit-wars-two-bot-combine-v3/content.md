...
def agent_Japan(obs):
    ...
    def _multi_factor_value(target, t_est, remaining_turns, is_static, is_early, is_late,
        ...
        """V22 + V28強化: phase別敵priority up, 終盤で中立無視."""
        ...
        # V28: phase別 hostile bonus 強化
        if target.owner not in (-1, owner_player):
            if is_early:
                value *= 1.45
            elif step < 200:     # 100-200 mid
                value *= 2.2     # 1.85 → 2.2 (敵奪還強化)
            else:                # 200+ late
                value *= 3.0     # 終盤 敵集中
        # V28: 終盤 中立は価値激減
        if target.owner == -1:
        ...
        if is_behind and target.owner == -1 and not is_static:
            value *= 0.92
        # V28: 最弱 player 惑星 priority +15% (4p kingmaking 回避逆転)
        if weakest_player is not None and target.owner == weakest_player:
            value *= 1.15
        # late (残少時) で敵弱なら elimination bonus
    ...
    def _compute_margin(target, t_est, is_static, is_four_player, is_finishing, player):
        """V21 preferred_send の margin 計算軽量版."""
        if target.owner == -1:
    ...
    def gen_moves(obs, player, enemy_margin_mult, room_factor, fire_threshold=FIRE_THRESHOLD):
        ...
        for f in raw_fleets:
            ...
            for mp in my_planets:
                ...
                d_line = rx * cx_ + ry * cy_
                if d_line <= 0:
                    continue
                side = abs(rx * (-cy_) + ry * cx_)
        ...
        # V28: 最弱 player (4p 用 kingmaking 回避, king狙い)
        weakest_player = None
    ...
    # 防御補給
        ...
        for mp in my_planets:
            ...
            for tp in planets:
                ...
                # V22: 多因子 value (V21 の target_value 軽量版)
                ...
                # losing時 敵攻撃prioritybonus は phase_by の中で処理
        ...
        for score, src_id, tgt_id, angle, needed, t_est in candidates:
            ...
            # V22: 多因子 margin (V21 preferred_send の軽量版)
            ...
            # V55: 上位 bot p90 MAX=3-4 に追従 (Shun_PI/gasin は 3-4, Lakhindar 6)
            # - 4p は 4
            # - is_early (step<40) は 2
            # - ships余裕時 (avail>50) は 3
            # - それ以外は 2 (V22相当)
            if is_four_player:
                max_moves = 3
...
# ===== 簡易シミュレータ =====
    def simulate(planets_init, fleets_init, my_moves, ang_vel, step_offset, comet_pids, n_steps, my_player, initial_planets_obs=None):
        """my_moves を player の手として実行、敵 noop で n_steps 進める.

        ★FIX (V16): rotation計算 は initial_planets_obs (game開始時pos) 起点.
        現在 planets の position は rotation 反映後なので、それを起点にすると累積ズレ.
        """
        ...
        # 自分の moves を fleet に変換 (interpreter の process_moves 同等)
        ...
        # n_steps シミュレート (敵 noop)
        # ★FIX (V16): interpreter は obs.step を rotation に使う (= step_offset + sim_step)
        for sim_step in range(n_steps):
            ...
            # 3. planet rotation (comet skip for sim simplicity)
            for p in planets:
    ...
    # 候補手生成
    candidates = []
    # C1: V11 default (margin*4, room*1.2)
    candidates.append(("v11_def", gen_moves(obs, player, 4, 1.2)))
    # C2: aggressive (margin*6, room*1.5) = V13
