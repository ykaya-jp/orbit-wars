## [MD]
# Orbit Wars 2026 — Submission

Heuristic agent built on the Doc-8 FFA mode-aware baseline (~800 LB score) with four additional improvements:

1. **Per-opponent proximity priority** — nearer enemies get higher priority in target scoring
2. **Enemy reinforcement margin** — account for enemy planets that can reinforce within 8 turns of our arrival
3. **Tie-hunt mode** — when behind but within 72% of the leader, accumulate ships for a guaranteed tie-win (engine awards reward=1 to ALL tied top players)
4. **Terminal fleet-launch phase** — in the last 8 turns, launch every spare ship at the nearest target (in-flight fleets count toward final score)
5. **Speed-threshold snap** — bump fleet sizes to hit speed tiers (43/78/137/232/385 ships) for faster arrivals
...
assert 'def agent(' in content
assert 'TIE_HUNT_SHIP_VALUE' in content
assert 'TERMINAL_LAUNCH_TURNS' in content
assert 'opponent_priority' in content
