...
def plan_moves(world, deadline=None):
    ...
    for src in world.my_planets:
        ...
        for target in world.planets:
            ...
            delays_to_test = WAIT_STRIKE_DELAYS if WAIT_STRIKE_ENABLED else (0,)
            best_capture = None

            for delay in delays_to_test:
                delayed_available = src_available + (delay * int(src.production) if src.owner == world.player else 0)
                future_turns = rough_turns + delay

                global_needed_future = world.min_ships_to_own_at(
                    target.id,
                    future_turns,
                    world.player,
                    planned_commitments=planned_commitments,
                )
                if global_needed_future <= 0 or global_needed_future > delayed_available:
                    continue
                if delay == 0 and opening_filter(target, rough_turns, global_needed_future, delayed_available, world, policy):
                    continue

                send_guess = preferred_send(target, global_needed_future, future_turns, delayed_available, world, modes, policy)
                plan = settle_plan(
                    src,
                    target,
                    delayed_available,
                ...
                total_turns = turns + delay

                if not time_filters_pass(target, total_turns, needed, delayed_available):
                    continue
                if send_cap < 1:
                    continue

                value = target_value(target, total_turns, "capture", world, modes, policy)
                if value <= 0:
                    continue

                score = apply_score_modifiers(
                    value / (send_cap + total_turns * ATTACK_COST_TURN_WEIGHT + 1.0),
                    target,
                    "capture",
                    world,
                )
            ...
            if best_capture is not None and best_capture.send_cap >= best_capture.needed:
                missions.append(
                    Mission(
                        kind="single",
                        score=best_capture.score,
                        target_id=target.id,
                        turns=best_capture.turns,
                        options=[best_capture],
                    )
                )
    ...
    for mission in missions:
        ...
        if mission.kind in ("single", "snipe", "rescue", "recapture", "reinforce", "crash_exploit"):
            ...
            if mission.kind == "reinforce":
                ...
                send = min(left + option.delay * int(src.production), option.send_cap)
                if send < option.needed:
                    continue
                spent_total[option.src_id] += send
                planned_commitments[target.id].append((option.turns, world.player, int(send)))
                continue
