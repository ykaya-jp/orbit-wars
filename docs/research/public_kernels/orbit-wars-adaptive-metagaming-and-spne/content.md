## [MD]
# Orbit Wars: Adaptive Metagaming and SPNE

---

This agent extends deterministic optimization by introducing N-Player Subgame Perfect Nash Equilibrium (SPNE) modeling. Rather than treating opponents as static environmental variables, this architecture profiles opponent decision-making patterns and modulates its visible strategic posture. It is designed to optimize performance in Free-For-All (FFA) configurations by mitigating coalition behaviors (the "Kingmaker Effect") and exploiting opponent heuristic assumptions.

---

## I. Opponent Profiling and Dynamic Parameter Recalibration

During the opening phase (**Turns 1–40**), the agent executes a Bayesian aggression profiler. Enemy fleet dispatches are tracked globally, updating a persistent `fleet_volume` counter within the `OPPONENT_FINGERPRINTS` registry.

At **Turn 41**, a recalibration protocol is triggered. The agent calculates the mean fleet volume across all opponents and categorizes them to adjust its Lanchester engagement parameters:

* **Aggressive Profile** (`fleet_volume > 1.4 × mean`): Opponent prioritizes early fleet commitment over defensive staging. The agent recalibrates its `LANCHESTER_YIELD_MAP` for this specific opponent to `0.9`, tightening its local density requirements and prioritizing frontier fortification.
* **Passive Profile** (`fleet_volume < 0.6 × mean`): Opponent favors garrison accumulation. The agent recalibrates to `1.2`, increasing acceptable risk to out-scale the opponent's production infrastructure before the late game.
* **Balanced Profile** (`fleet_volume ≈ mean`): Engagement constant is set to `1.1`.

These localized constants weight the margin calculations dynamically, allowing the agent to execute personalized optimization strategies against heterogeneous opponent types simultaneously.

## II. Dynamic Threat Masking Protocol

In FFA environments, exceeding a certain visible power threshold frequently triggers collective targeting by remaining opponents. To mitigate this, the agent utilizes a `SANDBAG_MODE` protocol to artificially suppress its apparent global threat share.

When the agent's actual relative `owner_strength` approaches coalition-triggering thresholds, it alters its target prioritization:
* High-production enemy captures are deprioritized in favor of low-visibility neutral acquisitions.
* The agent accumulates uncommitted strategic reserves within its garrisons, which are often unaccounted for in standard opponent threat-evaluation heuristics.
* This suppression protocol is terminated strictly when the SPNE lookahead identifies that a rival is approaching runaway dominance, at which point the accumulated reserves are deployed for a decisive interception.

## III. Evacuation Denial Logic

When the agent determines a friendly planet will fall to an overwhelming force (`DOOMED_EVAC_TURN_LIMIT = 24`, `DOOMED_MIN_SHIPS = 8`), it executes an evacuation sequence but intentionally leaves **exactly 1 ship** in the garrison.

This mechanic introduces deliberate friction into opponent logic systems:
* **Bonus Denial:** It forces a combat resolution state upon the arriving enemy, nullifying programmatic bonuses associated with uncontested, zero-garrison captures.
* **Simulator Desynchronization:** For opponent agents relying on rigid forward-projected arrival simulators, the 1-ship garrison alters the combat resolution timeline by exactly one discrete turn, causing downstream errors in their subsequent routing calculations.
* **State-Evaluation Friction:** The persistent ownership flag on a functionally lost planet generates noise in opponent spatial-clustering and target-selection heuristics.

## IV. Additional Architectural Specifications

* **Persistent State Tracking:** Maintains `OPPONENT_FINGERPRINTS` and `LANCHESTER_YIELD_MAP` as persistent module-level variables, retaining statistical intelligence across the full 500-turn episode.
* **SPNE Decision Horizon:** Actions are evaluated based on their impact on future decision nodes and equilibrium states, optimizing for terminal placement rather than strictly immediate ROI.
* **Inherited Tactical Infrastructure:** Retains the full algorithmic suite of its deterministic counterpart, including Network Flow synchronization, proactive defense scaling, and rear-echelon logistics, utilizing them as underlying mechanisms for its adaptive overarching strategy.
* **Real-Time Budget Management:** Utilizes the same `0.82 × actTimeout` soft-deadline parameters to ensure profiling and masking overhead does not exceed operational limits.

## [CODE]
```python
%%writefile submission.py
"""
THE PHANTOM METAGAME SOVEREIGN
Elite Adaptive Intelligence & Subgame Perfect Nash Equilibrium (SPNE) Architecture.
Incorporates Dynamic Slingshot Threat Masking, Opponent Fingerprinting,
and the Poisoned Chalice game-theoretic protocol.
"""
...
# Tracks the Bayesian aggression profile of opponents during the Epoch 1 (Turns 1-40).
OPPONENT_FINGERPRINTS = defaultdict(lambda: {"fleet_volume": 0, "aggressive": False})
# Mid-match calibrated Lanchester constants based on fingerprinting.
LANCHESTER_YIELD_MAP = {}
# Global toggle for threat suppression.
SANDBAG_MODE = False
...
def safe_angle_and_distance(sx, sy, sr, tx, ty, tr):
    # Blueprint Paradigm 5: Tangent routing for sun avoidance
    angle, start_x, start_y, end_x, end_y, hit_distance = actual_path_geometry(
        sx, sy, sr, tx, ty, tr
    )
    if segment_hits_sun(start_x, start_y, end_x, end_y):
        # Tangent routing around the sun
        d_center = dist(sx, sy, CENTER_X, CENTER_Y)
        if d_center > SUN_R + SUN_SAFETY:
            ang_to_center = math.atan2(CENTER_Y - sy, CENTER_X - sx)
            try:
                tangent_offset = math.asin((SUN_R + SUN_SAFETY) / d_center)
                ang1 = ang_to_center + tangent_offset
                ang2 = ang_to_center - tangent_offset
                d1 = abs((ang1 - angle + math.pi) % (2 * math.pi) - math.pi)
                d2 = abs((ang2 - angle + math.pi) % (2 * math.pi) - math.pi)
                safe_angle = ang1 if d1 < d2 else ang2
                # Path is roughly longer when going around
                return safe_angle, hit_distance * 1.15
            except ValueError:
                return None
        return None
...
def build_policy_state(world, deadline=None):
    """
    Constructs the global strategic policy. Approximates the SPNE by evaluating
    the Global Threat Matrix and applying perceived threat masking.
    """
    global SANDBAG_MODE
    ...
    # Paradigm 2: Threat Matrix Calculation
    # T_i = Ships + (Production * Turns_Remaining)
    threat_matrix = defaultdict(float)
    turns_left = world.remaining_steps
    for p in world.planets:
        if p.owner != -1:
            threat_matrix[p.owner] += p.ships + p.production * turns_left
    for f in world.fleets:
        threat_matrix[f.owner] += f.ships

    total_threat = sum(threat_matrix.values())

    # Apex Paradigm 1: Dynamic Slingshot (Threat Masking)
    # If our own power share exceeds the Kingmaker threshold (0.38), we artificially
    # suppress expansion to avoid coordinated opponent intervention.
    SANDBAG_MODE = False
    if total_threat > 0 and world.is_four_player:
        p_self = threat_matrix[world.player] / total_threat

        opponents = [o for o in threat_matrix.keys() if o != world.player]
        p_leader = (
            max([threat_matrix[o] / total_threat for o in opponents])
            if opponents
            else 0
        )

        # Disguise Release Trigger: We drop sandbagging if turn > 250 OR any opponent
        # crosses the dangerous leader threshold, requiring an industrial response.
        drop_disguise = world.step > 250 and p_leader > 0.38

        if p_self > 0.38 and world.step < 380 and not drop_disguise:
            SANDBAG_MODE = True
    ...
    if total_threat > 0:
        opponents = [o for o in threat_matrix.keys() if o != world.player]
        if opponents:
            leader_id = max(opponents, key=lambda o: threat_matrix[o])
            # If we are sandbagging, we don't want to trigger kingmaker against others
            # unless they are REALLY dominant, but actually kingmaker helps us stay 2nd.
            if threat_matrix[leader_id] / total_threat > 0.47:
                kingmaker_target = leader_id
    ...
    return {
        ...
        "kingmaker_target": kingmaker_target,
...
def target_value(target, arrival_turns, mission, world, modes, policy):
    # Blueprint Paradigm 4: Endgame Value Decay
    # Value(P, t) = (g_P * max(0, 500 - t - transit_time)) * exp(-3.0 * (t / 500))
    t = world.step
    transit_time = arrival_turns
    turns_profit = max(0, TOTAL_STEPS - t - transit_time)

    decay_multiplier = math.exp(-2.2 * (t / TOTAL_STEPS))
    value = (target.production * turns_profit) * decay_multiplier

    # Apex Paradigm 1: Sandbagging
    if policy.get("sandbag_mode", False):
        # Only sandbag if we are the clear leader (e.g., >1.2x next player)
        threats = policy.get("threat_matrix", {})
        if threats:
            my_threat = threats.get(world.player, 0)
            other_threats = [t for o, t in threats.items() if o != world.player]
            max_other = max(other_threats) if other_threats else 1
            if my_threat > max_other * 1.15:
                if target.owner != world.player:
                    ...
                    value *= 2.5  # Hide power

    # Blueprint Paradigm 2: Kingmaker Override

    if target.owner == policy.get("kingmaker_target", -1):
        value *= 5.0  # Massive boost for suicidal aggressive strikes

    # Keep some of the original heuristic logic for indirect wealth and status
    value += (
    ...
    # FFA-specific: Building a corner empire
    if modes["is_ffa"] and target.owner == -1 and is_safe_neutral(target, policy):
        value *= 1.15  # Prioritize safe growth in FFA
...
def calculate_lanchester_utility(s, target, transit_time, world):
    # Blueprint Paradigm 1: Lanchester's "Engage vs. Starve"
    # s: dispatch size
    # garrison_at_arrival = current_ships + (growth_rate * transit_time)
    garrison_at_arrival = target.ships
    if target.owner != -1:
        garrison_at_arrival += target.production * transit_time

    surplus = s - garrison_at_arrival
    future_yield = target.production * (TOTAL_STEPS - world.step - transit_time)

    # Apex Paradigm 2: Dynamic Fingerprinting
    yield_weight = LANCHESTER_YIELD_MAP.get(target.owner, 1.1)

    utility = (future_yield * yield_weight + surplus) - s
    weighted_utility = utility * (1.0 - (garrison_at_arrival / max(1, s)))
    return weighted_utility


def ships_for_speed(required_speed):
    # Blueprint Paradigm 3: Reverse-engineer ships for speed
    if required_speed <= 1.0:
        return 1
    if required_speed >= 6.0:
        return 1000
    exponent = (required_speed - 1.0) / 5.0
    # exponent = (ratio**1.5) => ratio = exponent**(1/1.5) = exponent**0.666
    ratio = exponent**0.666
    return int(math.ceil(1000**ratio))
...
def build_backstab_missions(world, policy, planned_commitments, modes):
    # Blueprint Paradigm 2: Apex Predator Backstab
    missions = []
    for target in world.planets:
        if target.owner == world.player:
            continue

        # Predicted capture turn (from world.fall_turn_map or similar)
        # Using the heuristic's fall_turn_map which considers incoming fleets
        fall_turn = world.fall_turn_map.get(target.id)
        if fall_turn is None or fall_turn > 100:
            continue

        t_capture = fall_turn
        t_sync = t_capture + 1

        # Calculate survivors (rough estimate)
        owner_at, ships_at = world.projected_state(target.id, t_capture)
        if owner_at == world.player or owner_at == -1:
            continue

        surviving_enemy = ships_at

        for src in world.my_planets:
            src_available = policy["attack_budget"].get(src.id, 0)
            if src_available < surviving_enemy + 1:
                continue

            # Distance and required transit time
            safe = world.plan_shot(src.id, target.id, src_available)
            if not safe:
                continue
            angle, _, tx, ty = safe
            hit_dist = dist(src.x, src.y, tx, ty) - src.radius - target.radius

            req_transit = t_sync - world.step
            if req_transit < hit_dist / 6.0:
                continue  # Too far

            req_speed = hit_dist / req_transit
            if req_speed < 1.0:
                continue  # Too slow

            s_throttle = ships_for_speed(req_speed)
            s_final = max(s_throttle, int(surviving_enemy) + 2)

            if s_final > src_available:
                continue

            # Double check ETA with s_final
            final_speed = fleet_speed(s_final)
            final_eta = int(math.ceil(hit_dist / final_speed))
            if world.step + final_eta != t_sync:
                continue

            value = target_value(target, final_eta, "backstab", world, modes, policy)
            score = value / (s_final + final_eta * SNIPE_COST_TURN_WEIGHT + 1.0)
            score *= 2.0  # High priority for backstabs
...
def plan_moves(world, deadline=None):
    ...
    # Blueprint Paradigm 5: Knapsack Opening (Turns 1-20)
    if 1 <= world.step <= 20:
        for src in world.my_planets:
            avail = int(src.ships)
            neutral_candidates = []
            for target in world.neutral_planets:
                aim = world.plan_shot(src.id, target.id, avail)
                if aim:
            ...
            # 0/1 Knapsack greedy approx
            neutral_candidates.sort(key=lambda x: x["prod"] / x["cost"], reverse=True)
            for cand in neutral_candidates:
                if spent_total[src.id] + cand["cost"] <= avail:
                    moves.append([src.id, float(cand["angle"]), int(cand["cost"])])
                    spent_total[src.id] += cand["cost"]
        return moves

    # Blueprint Paradigm 4: Scorched Earth (Turn 460+)
    if world.step >= 460:
        for src in world.my_planets:
            avail = int(src.ships)
            if avail > 0:
                enemy_capitals = sorted(
                    world.enemy_planets, key=lambda p: p.production, reverse=True
                )
                if enemy_capitals:
                    target = enemy_capitals[0]
                    aim = world.plan_shot(src.id, target.id, avail)
                    if aim:
                        moves.append([src.id, float(aim[0]), avail])
                        spent_total[src.id] += avail
        return moves
    ...
    missions.extend(build_backstab_missions(world, policy, planned_commitments, modes))
    ...
    for src in world.my_planets:
        ...
        for target in world.planets:
            ...
            if global_needed <= src_available:
                ...
                if send_cap >= needed:
                    # Blueprint Paradigm 1: Lanchester's "Engage vs. Starve"
                    l_util = calculate_lanchester_utility(
                        send_cap, target, turns, world
                    )
                    if l_util >= 0:
                        missions.append(
                            Mission(
    ...
    for mission in missions:
        ...
        if mission.kind in (
            ...
            elif mission.kind == "backstab":
                # For backstab, we already calculated the throttled ship count
                plan = (
                    option.angle,
                    option.turns,
                    None,
        ...
        # Blueprint Paradigm 3: Tsunami Strike (Synchronized Arrival)
        t_sync = mission.turns
        reaimed = []
        total_ships_committed = 0

        # We need to reach t_sync from each source
        possible = True
        for option in mission.options:
            src = world.planet_by_id[option.src_id]
            left = source_attack_left(option.src_id)
            ...
            if req_transit <= 0:
                ...
                break
            req_speed = hit_dist / req_transit
            if req_speed > 6.0 or req_speed < 1.0:
                possible = False
                break

            s_req = ships_for_speed(req_speed)
            if s_req > left:
                possible = False
                break

            reaimed.append((src.id, angle, s_req))
            total_ships_committed += s_req

        if not possible:
            continue

        # Final check if total ships are enough
        owner_after, ships_after = world.projected_state(
            target.id,
            t_sync,
            planned_commitments=planned_commitments,
            extra_arrivals=[(t_sync, world.player, s) for _, _, s in reaimed],
        )
        if owner_after != world.player:
            continue

        for src_id, angle, send in reaimed:
            actual = append_move(src_id, angle, send)
            if actual > 0:
                planned_commitments[target.id].append(
                    (t_sync, world.player, int(actual))
                )
    ...
    if live_doomed:
        ...
        for planet in world.my_planets:
            ...
            # Apex Paradigm 3: The Poisoned Chalice
            # Game-theoretic evacuation. By leaving exactly 1 ship behind, we force
            # a combat resolution turn on the opponent. This strips their "peaceful
            # capture" velocity bonus and forces them to re-verify kinematic arrival windows.
            available_now = max(0, source_inventory_left(planet.id) - 1)
            if available_now < 1:
                continue
...
def agent(obs, config=None):
    global LANCHESTER_YIELD_MAP, SANDBAG_MODE
    start_time = time.perf_counter()
    world = build_world(obs)

    # Initialization of Dynamic Parameters
    for p in world.planets:
        if p.owner not in (-1, world.player) and p.owner not in LANCHESTER_YIELD_MAP:
            LANCHESTER_YIELD_MAP[p.owner] = 1.1

    # Apex Paradigm 2: Opponent Fingerprinting (Turns 1-40)
    # Samples dispatched fleet volume to profile opponent aggression profiles.
    if world.step <= 40:
        for fleet in world.fleets:
            if fleet.owner != world.player:
                OPPONENT_FINGERPRINTS[fleet.owner]["fleet_volume"] += fleet.ships

    # Recalibration Window: Transition from profile sampling to dynamic adaptation.
    if world.step == 41:
        others = [f for f in OPPONENT_FINGERPRINTS.values()]
        if others:
            avg_vol = sum(f["fleet_volume"] for f in others) / len(others)
            for owner, stats in OPPONENT_FINGERPRINTS.items():
                if stats["fleet_volume"] > avg_vol * 1.4:
                    # Counter Hyper-Aggression: prioritize defense and local density.
                    LANCHESTER_YIELD_MAP[owner] = 0.9
                elif stats["fleet_volume"] < avg_vol * 0.6:
                    # Counter Turtles: increase expansion weight to out-scale.
                    LANCHESTER_YIELD_MAP[owner] = 1.2
                else:
                    LANCHESTER_YIELD_MAP[owner] = 1.1
