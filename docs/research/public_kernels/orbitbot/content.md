...
# FFA Third-Party Opportunism (romantamrazov 1224 features)
...
EARLY_STATIC_NEUTRAL_SCORE_MULT = 1.3
...
REINFORCE_MAX_SOURCE_FRACTION = 0.7
...
REAR_DISTANCE_RATIO = 1.2
REAR_STAGE_PROGRESS = 0.8
REAR_SEND_RATIO_TWO_PLAYER = 0.6
...
MULTI_ENEMY_PROACTIVE_RATIO = 0.14
...
def target_value(target, arrival_turns, mission, world, modes, policy):
    ...
    # FFA Third-Party Opportunism (romantamrazov FFA features)
    # Only meaningful in 4+ player games
    if world.is_four_player and target.owner != world.player:
        ...
        if enemy_arrivals:
            # Group by enemy owner
            enemy_forces = {}
            for eta, owner, ships in enemy_arrivals:
                enemy_forces[owner] = enemy_forces.get(owner, 0) + ships

            num_enemies_fighting = len(enemy_forces)
            forces = list(enemy_forces.values())

            # BLOOD_IN_WATER: Multiple enemies attacking same target (chaos = opportunity)
            if num_enemies_fighting >= 2:
                value *= BLOOD_IN_WATER_VALUE_MULT
                # LET_THEM_FIGHT: Enemies are roughly matched — they'll bleed each other
                # Discount urgency since we may want to wait for them to weaken each other
                if len(forces) == 2:
                    ratio = min(forces) / max(forces) if max(forces) > 0 else 0
                    if ratio > 0.5:
                        value *= LET_THEM_FIGHT_PENALTY
            # EXPOSED_PLANET: Target has thin garrison during enemy combat (weak defender = opportunity)
            elif num_enemies_fighting == 1:
                enemy_ships = forces[0]
                # Planet is exposed if garrison is small relative to enemy attacking force
                if target.ships < enemy_ships * 0.8:
...
def plan_moves(world, deadline=None):
    ...
    # as slow solo attackers. In TOTAL_WAR phase, drain aggressively forward.
    if (
        (world.enemy_planets or world.neutral_planets)
        and len(world.my_planets) > 1
        and (not world.is_late or world.is_total_war)
        ...
        if safe_fronts:
            ...
            # TOTAL_WAR: Maximize forward push — avoid leaving stranded ships
...
with open("submission.py") as f:
    code = f.read()
print(f"Submission has {len(code.split(chr(10)))} lines")
assert len(code.split(chr(10))) > 100, "Too short!"
