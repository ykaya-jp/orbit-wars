...
Orbit Wars ensemble-training notebook packaging the current local submission agent.
...
ATTACK_COST_TURN_WEIGHT = 0.551
SNIPE_COST_TURN_WEIGHT = 0.346
...
HOSTILE_TARGET_VALUE_MULT = 1.792
OPENING_HOSTILE_TARGET_VALUE_MULT = 1.224
...
REAR_SEND_RATIO_TWO_PLAYER = 0.595
REAR_SEND_RATIO_FOUR_PLAYER = 0.616
...
with open("submission.py", encoding="utf-8") as f:
    code = f.read()
print({"lines": len(code.splitlines()), "chars": len(code)})
assert "def agent" in code
