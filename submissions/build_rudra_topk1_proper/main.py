...
# ============================================================================
# topk1 wrapper (= keep only the LARGEST-SHIPS move per turn)
# Hypothesis: top-tier players use few decisive large-fleet moves (kovi 0.10 launch/step).
# Source: konbu17/train-submit-v4-ml-validator-topk1-tutorial (claim +75 LB).
# Confirmed in our konbu17_topk1 submission (LB 961 with topk1, 989 without per 5/10).
# Day 2 episode analysis: large-fleet wins 4/5; small-fleet loses (= 5/11 doc).
# ============================================================================

_orig_agent_topk1 = agent

def agent(obs, *args, **kwargs):
    moves = _orig_agent_topk1(obs, *args, **kwargs)
    if not moves or len(moves) <= 1:
        return moves
    # Each move is [from_id, angle, ships]; pick the one with max ships
    try:
