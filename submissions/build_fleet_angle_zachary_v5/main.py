...
def agent(obs):
    ...
    for src in my:
        ...
        # v5 = mid-late fleet boost (bovard 真因 deep dive):
        # step 300-450 で Top tier fleet mean = 303 vs other 91 (= 3.4x)、
        # step 400+ で 500+ ships launch 比率 = top 15.6% vs other 4.4% (= 3.5x)。
        # 既存 phase の send size (= 30-50% of ships) を「step 300-460 で 70%+ boost」 で
        # Top tier の big-fleet focus を再現。 expand phase は除外 (= early game neutral capture
        # は small fleet OK)。
        if 300 <= step < 460 and phase not in ('endgame_pileup', 'expand', 'opportunistic'):
            boosted = int(src['ships'] * 0.70)
            send = max(send, boosted)
            send = min(send, src['ships'] - 1)
