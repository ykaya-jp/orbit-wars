...
# topk2 wrapper (= 2 largest, balance offense/defense)
_orig_rudra_topk2 = agent
def agent(obs, *args, **kwargs):
    moves = _orig_rudra_topk2(obs, *args, **kwargs)
    if not moves or len(moves) <= 2: return moves
    try:
        sorted_moves = sorted(moves, key=lambda m: -(m[2] if len(m) > 2 else 0))
