...
_THREAT_HORIZON_TURNS = 60  # ETA window for fleet trajectory hit detection


def planet_under_threat(p_id, fleets, planets, player, omega):
    """fleet.angle trajectory hit + Let-them-fight multi-owner combat resolution.

    v2 (fleet.angle): trajectory hit predict (= base agent.py:649-650 流)。
    v3 (Let-them-fight, Tier 3 #1): 4P FFA で他 player 同士が same target を狙うとき、
    engine combat resolution (orbit_wars.py:650-665) は `top - second = survivor`、
    same ships なら 0。 incoming を **owner 別集計** して top - second を threat 値とする
    (= 他 player 同士で食い合い、 我家への真の damage 軽減)。
    """
    target = planets[p_id]
    tgt_radius_sq = target['radius'] * target['radius']
    is_orbiting_p = math.hypot(target['x'] - SUN_X, target['y'] - SUN_Y) + target['radius'] < 48.0

    incoming_by_owner: dict = {}
    ...
    for f in fleets.values():
        ...
        dir_x = math.cos(f['angle'])
        dir_y = math.sin(f['angle'])
        ...
        if perp_sq >= tgt_radius_sq:
            continue
        hit_dist = max(0.0, proj - math.sqrt(max(0.0, tgt_radius_sq - perp_sq)))
        hit_time = hit_dist / max(speed, 1e-6)
        if hit_time > _THREAT_HORIZON_TURNS:
            continue

        if is_orbiting_p:
            pred_x, pred_y = predict_orbit(target['x'], target['y'], omega, hit_time)
            dx2 = pred_x - f['x']
            dy2 = pred_y - f['y']
            proj2 = dx2 * dir_x + dy2 * dir_y
            if proj2 < 0:
                continue
            perp2_sq = dx2 * dx2 + dy2 * dy2 - proj2 * proj2
            if perp2_sq >= tgt_radius_sq:
                continue
            hit_dist2 = max(0.0, proj2 - math.sqrt(max(0.0, tgt_radius_sq - perp2_sq)))
            hit_time2 = hit_dist2 / max(speed, 1e-6)
            if hit_time2 > _THREAT_HORIZON_TURNS:
                continue

        owner = f['owner']
        incoming_by_owner[owner] = incoming_by_owner.get(owner, 0) + f['ships']

    if not incoming_by_owner:
        return 0

    # Let-them-fight: combat resolution = top - second = 我家への true survivor threat
    sorted_owners = sorted(incoming_by_owner.items(), key=lambda x: -x[1])
    if len(sorted_owners) >= 2:
        top_ships = sorted_owners[0][1]
        second_ships = sorted_owners[1][1]
        # engine line 650-665: same → 0, different → top - second
        return top_ships - second_ships if top_ships != second_ships else 0
    return sorted_owners[0][1]
...
def agent(obs):
    ...
    # === End-game ship pile-up (= engine audit #4, bovard 真因解析と整合) ===
    # step >= ENDGAME_THRESHOLD で all-ship total attack
    # 根拠: engine termination = step >= 498 で score = sum(ships) per player, max のみ +1 reward
    # bovard 280万row: top tier は step 400+ で ships 1828 (= our 946 の 1.93x)、
    # = late game ships 蓄積 + 1-shot 総攻撃で 1st position 取得が必須
    ENDGAME_THRESHOLD = 460  # remaining 38 turns で総攻撃に切替
    is_endgame = step >= ENDGAME_THRESHOLD

    if is_endgame and enemy:
        phase = 'endgame_pileup'
    elif smash_targets:
        phase = 'smash'
    ...
    # v4 = mid-game ship saving (= bovard 真因分析: Top tier は step 100-300 で attack rate を
    # middle の half に抑制、 ships を planets に蓄積して mid-late で大 fleet 攻撃)。
    # Phase saving (= step 100-300) で attack threshold を step-dependent に厳格化。
    if 100 <= step <= 300 and phase not in ('counter_attack', 'smash', 'endgame_pileup'):
        _min_launch_ships = 40  # saving phase = small fleet 連発禁止
    elif step > 460:
        _min_launch_ships = 10  # endgame = restore aggressive
    else:
        _min_launch_ships = 10  # early/late = base

    for src in my:
        if src['id'] in in_flight_from:
            continue

        if src['ships'] < _min_launch_ships:
            continue
        ...
        if phase == 'endgame_pileup' and enemy:
            # End-game total attack: 最大 production*ships の enemy 1 つに all sources を集中
            # score = ships + 10 * prod (= production が長期生産で重要、 既存 ships は immediate value)
            max_enemy = max(enemy, key=lambda t: t['ships'] + 10 * t['prod'])
            candidates = [max_enemy]
        elif phase == 'smash':
        ...
        if phase == 'endgame_pileup':
            # All-in: 98% of ships (= reserve 2% for last-second contingency)
            send = int(src['ships'] * 0.98)
            send = max(send, ships_needed_for_takeover(tgt['ships'], tgt['prod'], tt, tgt['owner']))
            send = min(send, src['ships'] - 1)
        elif phase == 'smash':
...
def agent(obs, *args, **kwargs):
    moves = _orig_zachary(obs, *args, **kwargs)
    if not moves or len(moves) <= 1: return moves
    try: return [max(moves, key=lambda m: m[2] if len(m) > 2 else 0)]
    except: return moves
