...
def planet_under_threat(p_id, fleets, planets, player, omega):
    """fleet.angle 直読 trajectory hit 判定 (= base agent.py:649-650 流の正確な defense).

    Day 2 review fix: 旧版は "fleet が最も近い planet を target と推測" の nearest-target
    heuristic で、 fleet.angle 全可視を活用してなかった (= LB +100-200 機会損失)。
    本版は fleet trajectory (dir_x = cos(angle), dir_y = sin(angle)) と planet 中心
    距離の射影 (proj > 0) + 直交距離 (perp_sq < radius**2) で正確に hit 判定する。
    Orbital planet は predict_orbit で hit_time の位置補正を反復。
    """
    target = planets[p_id]
    ...
    for f in fleets.values():
        ...
        # 1st pass: assume target static for hit_time estimate
        dx = target['x'] - f['x']
        dy = target['y'] - f['y']
        ...
        # 2nd pass: orbital planet なら predicted position で再検証
        if is_orbiting_p:
