    ...
    if moving_targets:
        ...
        for turn in range(1, max_turns + 1):
            ...
            for planet in moving_targets:
                # v37's integer sweep is the proven moving-target win, but it
                # can miss fast tangent hits when both fleet and planet cross
                # between ticks.  Keep the same conservative ETA ceiling while
                # sampling the in-tick fleet segment against midpoint/end target
                # positions.  This improves the arrival ledger without changing
                # launch angles or creating fractional-turn commands.
                for substep in range(1, FLEET_SWEEP_SUBSTEPS + 1):
                    sample_turn = turn - 1.0 + (float(substep) / FLEET_SWEEP_SUBSTEPS)
                    if sample_turn > best_time:
                        break
                    sample_x = fleet.x + dir_x * speed * sample_turn
                    sample_y = fleet.y + dir_y * speed * sample_turn
                    pos = predict_target_position(
                        planet,
                        sample_turn,
                        initial_by_id,
